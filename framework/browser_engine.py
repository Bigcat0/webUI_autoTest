...
class BrowserEngin(object):
    ...
    def open_browser(self):
        ...
        logger.info("选择%s浏览器"%browser)#you had selectt %s browser
        url=config.get("testServer","URL")
        logger.info("测试URL是:%s" %url)  # the test server url is

        if browser =="Firefox":
            driver=webdriver.Firefox(executable_path=self.firefox_driver_path)#启动Firefox
            logger.info("启动火狐浏览器")
        elif browser =="Chrome":
